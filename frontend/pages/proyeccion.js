...
export default function ProjectionPage() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (res.ok) setSettings(await res.json()); else setSettings({});
      } catch (err) { console.error("Error al cargar configuración.", err); setSettings({}); }
    };
    fetchSettings();
  ...
  if (settings === null) {
    return <div style={{...styles.baseContainer, display: 'flex', alignItems: 'center', justifyContent: 'center'}}><h1>Cargando...</h1></div>
  }
